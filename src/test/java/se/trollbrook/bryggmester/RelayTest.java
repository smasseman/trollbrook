...
public class RelayTest {
	...
	public void testListenerNotifications() throws InterruptedException {
		Relay r = new Relay("TestRelay");
		RelayListener listener = Mockito.mock(RelayListener.class);
		r.addListener(listener);

		r.setState(RelayState.ON);
		Thread.sleep(500);
		Mockito.verify(listener, Mockito.times(1)).eventNotification(RelayState.ON);
		...
		r.setState(RelayState.OFF);
		Thread.sleep(500);
		Mockito.verify(listener, Mockito.times(1)).eventNotification(RelayState.OFF);

		r.setState(RelayState.ON);
		Thread.sleep(500);
		Mockito.verify(listener, Mockito.times(2)).eventNotification(RelayState.ON);
	}
